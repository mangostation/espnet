...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        if args.mtlalpha < 1:
            self.pos_enc = PositionalEncoding(args.adim, 0.1)
            self.pds = PDS(attention_dim=args.adim)
        ...
        # ch_bert = BertModel.from_pretrained("/work/m11115119/espnet/egs/aishell/asr1/bert_BertASR/bert_permutation/checkpoint-10000")
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        pe = torch.zeros((hs_pad.shape[0], 60, self.adim)).to(hs_pad.device)
        pe = self.pos_enc(pe)
        hs_pad, hs_mask = self.pds(pe, hs_pad, hs_mask)
        pred_pad_de, pred_mask = self.decoder(hs_pad, hs_mask)
        if ys_pad != None:
            # ys_in_pad, ys_out_pad = add_sos_eos(
            #     ys_pad, self.sos, self.eos, self.ignore_id
            # )
            pred_pad = self.classifier(pred_pad_de)
            self.pred_pad = pred_pad
            ys_out_pad = ys_pad
            
            target_60 = torch.full((ys_out_pad.size(0),60), 0).to(ys_out_pad.device)
            ...
            self.acc = th_accuracy(
                pred_pad.view(-1, self.odim), ys_out_pad, ignore_label=0
            )
    ...
    def encode(self, x):
        ...
        pe = self.pos_enc(pe)
        pds_output, *_ = self.pds(pe, enc_output, None)
        dec_output, *_ = self.decoder(pds_output, None)
