...
concatjson.py ${dir}/data_bert_tmp.*.json > ${dir}/data_bert_tmp.json

if [ $num_spkrs -eq 1 ]; then
  json2trn_bert.py ${dir}/data_bert_tmp.json ${dic} --num-spkrs ${num_spkrs} --refs ${dir}/ref.trn --hyps ${dir}/hyp.trn
  ...
  json2trn.py ${dir}/data_bert_tmp.json ${dic} --num-spkrs ${num_spkrs} --refs ${ref_trns} --hyps ${hyp_trns}
